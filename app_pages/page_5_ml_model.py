...
from src.machine_learning.evaluate_regression import regression_evaluation, regression_performance, regression_evaluation_plots

def page_5_ml_model_body():
    """
    Streamlit page handler for displaying the machine learning model's performance metrics,
    ...
    feature_importance = plt.imread(f"outputs/ml_pipeline/predict_sale_price/{version}/features_importance.png")
    ...
    st.info(
        "A machine learning model has been developed to predict house sale prices in Ames, Iowa.\n "
        "The model utilizes the ExtraTreesRegressor algorithm, a linear regressor.\n\n We initially used "
        "a regressor model to predict the sale price and attempted to improve its performance with PCA, "
        "but this did not yield better results.\n\n The initial dataset was comprised of 23 features. We dropped "
        "two variables that had approximately 90% missing data. Additionally, we removed GarageYrBlt due to "
        "its strong correlation with YearBuilt and YearRemodAdd, which caused redundancy. Further feature "
        "engineering and smart correlated selection led to the removal of another variable, **1stFlrSF**.\n\n The "
        "model was then tuned using a hyperparameter as shown in the ML pipeline below,"
    ...
    regression_performance(X_train=X_train, y_train=y_train, X_test=X_test, y_test=y_test, pipeline=sale_price_pipeline)
