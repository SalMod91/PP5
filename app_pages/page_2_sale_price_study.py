...
def page_2_sale_price_study_body():
    ...
    It loads data, displays business requirements,
    and visualizes correlations and PPS.
    """
    ...
    st.success(
        ...
        "* The client is interested in discovering "
        "how the house attributes correlate "
        "with the **Sale Price**. Therefore, "
        "the client expects data visualizations"
        " of the correlated variables against the sale price to show that."
    )
    ...
    if st.checkbox("Inspect Dataset"):
        st.write(
            ...
            "columns.\n\n"
            "Displayed below are the first 10 rows of the dataset.")
        ...
        st.write(
            ...
            - **OverallCond**: Rates the overall condition of the house
                ...
                - *2*: Poor
                - *1*: Very Poor
            - **OverallQual**: Rates the overall material/finish of the house
                - *10*: Very Excellent
                - *9*: Excellent
                - *8*: Very Good
                - *7*: Good
    ...
    st.write(
        ...
        "both Pearson and Spearman methods, "
        "identifying the most significant"
        " features for predicting Sale Price as:\n\n "
        "- **Overall Quality**\n"
        "- **Ground Living Area**\n"
        "- **1st Floor Area**\n"
        "- **Garage Area**\n"
        ...
        "- **Year Built**\n"
        "- **Year Remodeled**\n\n"
        "The features listed above exhibit strong linear"
        " and monotonic relationships with Sale Price, "
        "signifying that:\n"
        "- The sale price is higher for homes with higher overall quality.\n"
        "- The sale price increases with the size of the house,"
        "indicating larger houses tend to sell for more.\n"
        "- The sale price is higher for recently built houses.\n\n"
        "Visualizations below further illustrate these"
        " correlations through detailed plots "
        "between the identified features and the target variable.\n\n"
    )

    # Display a warning message about potential delays
    st.warning(
        "**Note**: Due to the heavy computation requirements,"
        " the visualizations may "
    ...
    st.info(
        ...
        "- A coefficient of 1 implies a perfect "
        "positive linear relationship, where one "
        ...
        "- A coefficient of 0 indicates no linear"
        "correlation, suggesting no linear "
        "dependence between the variables.\n\n"
        "Below, there is an illustration through"
        " a heatmap and barplots showing "
    ...
    st.info(
        ...
        " positive monotonic relationship, where "
        "the ranking of one variable increases with"
        " the ranking of the other.\n"
        "- A coefficient of -1 signifies a perfect negative"
        " monotonic relationship, where "
        "the ranking of one variable decreases as the"
        " ranking of the other increases.\n"
        "- A coefficient of 0 indicates no monotonic "
        "correlation, suggesting that the "
        "variables do not maintain a consistent direction"
        " in their relationship.\n\n"
        "Below, there is an illustration through a"
        " heatmap and barplots showing "
        "the features most strongly correlated to the Sale Price."
    )
    ...
    st.info(
        ...
        "The PPS heatmap is a visual tool that helps "
        "identify which features are good predictors "
        "for a specific variable. To use this heatmap"
        " effectively, follow these steps:\n\n"
        "- **Locate the Target Variable**: Find the variable you"
        " are interested in on the Y-axis of the heatmap. "
        "This is the variable for which you want to identify"
        " good predictors.\n"
        "- **Analyze Predictors**: Look across the corresponding"
        " row on the X-axis to see which features "
        "are good predictors of the target variable. "
        "The heatmap cells are colored based on the PPS score: "
        "darker colors typically indicate stronger predictive power.\n\n"
        "**Rule of Thumb for Good Predictive Power**:\n"
        "- A PPS Score above 0.2 is often considered "
        "to have moderate predictive power, indicating "
        "that the variable contributes valuable information "
        "that might be significant in predictive models.\n"
        "- A PPS Score above 0.5 generally indicates "
        "strong predictive power, suggesting a substantial "
        "influence of the predictor on the target variable.\n\n"
        "**Note on Diagonal Entries in Heatmaps**:\n\n"
        "The diagonal cells in a PPS heatmap, "
        "where a variable intersects with itself, "
        "always score 1 because a variable perfectly predicts itself. "
        "These entries should be ignored "
        "as they do not provide insights into the predictive "
        "relationships between different features.\n\n"
        "Below, an illustration through a heatmap displays "
        "the variables with the highest Predictive Power Scores, "
        "highlighting key relationships both "
    ...
    st.write(
        ...
        Bivariate analysis involves examining the relationships between two
        variables.
        To better understand the positive relationships between features most
        correlated
    ...
    st.info(
        ...
        **About Regression Plots**:
        Regression plots show the relationship between two variables with data
        points and a line of best fit.
        This line helps us see the trend and understand how changes in one
        variable could influence the other.
        They are especially useful for seeing how features linked to sale price
        impact its values.
        """
    )
...
def calculate_pps(df):
    """
    Calculates the Predictive Power Score (PPS) matrix
    and displays it using a heatmap.
    """
    ...
    pps_score_stats = pps_matrix_raw.query("ppscore < 1").filter(
        ['ppscore']
    ).describe().T

    heatmap_pps(
        df=pps_matrix, threshold=0.15, figsize=(12, 10), font_annot=10
    )
...
def plot_correlation_bar(df, method='pearson'):
    """
    Plots a bar chart of the strongest correlations
    with Sale Price using the specified correlation method.
    """
    df_corr = df.corr(method=method)['SalePrice'].drop('SalePrice')

    threshold = 0.5
    significant_corrs = df_corr[abs(df_corr) > threshold].sort_values(
        ascending=False
    )

    plt.figure(figsize=(12, 10))
    sns.barplot(
        x=significant_corrs.values,
        y=significant_corrs.index,
        palette="viridis"
    )
    plt.title(
        f'Top Correlations with Sale Price using {method.title()} Correlation'
    )
